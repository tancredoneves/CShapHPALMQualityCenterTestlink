...
namespace LinqExcel
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string[] linhas = casetest.Select(t => t.NOMEDOTESTE).Take(2).ToArray();
            string[] pre = casetest.Select(t => t.NOMEDOTESTE).Take(2).ToArray();
            string[] etapa = casetest.Select(t => t.ETAPA.Replace("Etapa ", "")).Take(2).ToArray();
            string[] des = casetest.Select(t => t.DESCRICAO).Take(2).ToArray();
            string[] res = casetest.Select(t => t.RESULTADOESPERADO).Take(2).ToArray();
